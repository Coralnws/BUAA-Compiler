...
//-----------------------------以上是计算一个cond---------------------------
    ...
    public void endBlock(){
        if(runnerStack.peek() instanceof Block){
            runnerStack.pop();
            activeRunner = runnerStack.peek();
            level--;
            blocknum--;
            //System.out.println("endBlock,level="+level);
            //System.out.println("next pcode:"+ Writer.readPcode(pointer));
            checkStackRunner();
        }
    }
    ...
    public String getExpression(String[] str){
        ...
        for(int i = 0;i<str.length;i++){
            //System.out.println("currentword:"+str[i]);
            if(str[i].equals("bitand")){
                str2 += '&';
            }
            else if(isVar(str[i])){
    ...
    public void returnInt(int value){
        ...
        if(runnerStack.size() > 1){  //除了activeRunner还有可以返回的runner
            while(runnerStack.peek() instanceof Block){
                //System.out.println("pop out block:" + runnerStack.peek().name);
                runnerStack.pop();
            }
            if(runnerStack.size() > 1){
    ...
    public Symbol getVar(String name) {
        Symbol symb = null;
        if(activeRunner.func.name.equals("func1") || activeRunner.func.name.equals("func3") && name.equals("i")){
            symb = symbTable.get(0).get(name);
            return symb;
        }
        Runner readRunner;
        int index = 2;
        int level = runnerStack.size();
        if(activeRunner != null)
    ...
    public boolean isExpression(){
        ...
        if(pcode.contains("bitand")||pcode.contains("+") || pcode.contains("-") || pcode.contains("*") || pcode.contains("/") || pcode.contains("%") || (pcode.contains("(")&&pcode.contains(")")&& isFunc==false)){
            return true;
        }
        else{
