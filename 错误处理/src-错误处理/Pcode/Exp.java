...
public class Exp extends PcodeGenerator{
    ...
    public Exp() throws IOException {
        ...
        Word word = SaveContent.getWord(wordIndex);
        while(!word.typeCode.equals("SEMICN") && !word.typeCode.equals("COMMA")){
            if(word instanceof lexerWord){
                if(word.content.equals(")")){
                    break;
                }
                currentExp += " "  + word.content;
            }
            wordIndex++;
            word = SaveContent.getWord(wordIndex);
        }
